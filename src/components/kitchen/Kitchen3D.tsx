...
const UNIT_POSITIONS: Record<string, { 
  pos: [number, number, number]; 
  size: [number, number, number]; 
  type: 'cabinet' | 'appliance' | 'glass' | 'drawer' | 'freezer' | 'pantry' | 'fridge' | 'base-drawer' | 'upper-split' | 'sink-cabinet';
  ...
  "PEN1-UPPER-1": { pos: [3.2, 4.5, 3.45], size: [1.25, 2.5, 1], type: 'upper-split', handleDir: 'left' },
  "PEN1-UPPER-2": { pos: [3.2, 4.5, 1.95], size: [1.25, 2.5, 1], type: 'upper-split', handleDir: 'left' },
  "PEN1-UPPER-3": { pos: [3.2, 4.5, 0.45], size: [1.25, 2.5, 1], type: 'upper-split', handleDir: 'left' },
  "PEN1-UPPER-4": { pos: [3.2, 4.5, -1.05], size: [1.25, 2.5, 1], type: 'upper-split', handleDir: 'left' },

  // PENINSULA 2 - 18" deep, connected
  ...
  "PEN2-UPPER-1": { pos: [-10.8, 4.5, 3.45], size: [1.25, 2.5, 1], type: 'glass', handleDir: 'right' },
  "PEN2-UPPER-2": { pos: [-10.8, 4.5, 1.95], size: [1.25, 2.5, 1], type: 'glass', handleDir: 'right' },
  "PEN2-UPPER-3": { pos: [-10.8, 4.5, 0.45], size: [1.25, 2.5, 1], type: 'glass', handleDir: 'right' },
  "PEN2-UPPER-4": { pos: [-10.8, 4.5, -1.05], size: [1.25, 2.5, 1], type: 'glass', handleDir: 'right' },
    ...
    <group position={position}>
      ...
      {(isSelected || hovered) && (
        <Html position={[0, size[1] / 2 + 0.15, 0]} center distanceFactor={10} style={{ pointerEvents: "none" }}>
          <div className={`rounded px-1 py-0.5 text-[8px] font-medium whitespace-nowrap ${
...
function UnitMesh({ unitId, label, zones, selectedZoneId, onClick }: UnitMeshProps) {
    ...
    return (
      <group position={pos}>
        ...
        <RoundedBox args={[size[0], size[1], size[2]]} radius={0.03} smoothness={4}>
          <meshStandardMaterial color={COLORS.applianceWhite} roughness={0.3} metalness={0.1} transparent opacity={0.3} />
        </RoundedBox>
        {/* 5 clickable zones */}
        {zones.map((zone, i) => (
          <ZoneMesh
        ...
        <Html position={[0, size[1] / 2 + 0.3, 0]} center distanceFactor={10} style={{ pointerEvents: "none" }}>
          <div className="rounded px-1.5 py-0.5 text-[10px] font-medium whitespace-nowrap bg-card/80 text-foreground border border-border">{label}</div>
        </Html>
      </group>
    );
  }
  ...
  if (type === 'fridge') {
    ...
    return (
      <group position={pos}>
        {/* Main body */}
        <RoundedBox args={[size[0], size[1], size[2]]} radius={0.03} smoothness={4}>
          <meshStandardMaterial color={COLORS.applianceBlack} roughness={0.2} metalness={0.5} transparent opacity={0.4} />
        </RoundedBox>
        
        {/* Interior zones */}
        {/* Top shelf left */}
        <ZoneMesh zoneId="FridgeTopShelfLeft" position={[-size[0] / 4, size[1] / 2 - 0.6, 0]} size={[size[0] / 2 - 0.1, 0.8, size[2] - 0.3]} color="#e8e4e0" isSelected={selectedZoneId === "FridgeTopShelfLeft"} onClick={onClick} label="Top Left" />
        {/* Top shelf right */}
        <ZoneMesh zoneId="FridgeTopShelfRight" position={[size[0] / 4, size[1] / 2 - 0.6, 0]} size={[size[0] / 2 - 0.1, 0.8, size[2] - 0.3]} color="#e8e4e0" isSelected={selectedZoneId === "FridgeTopShelfRight"} onClick={onClick} label="Top Right" />
        {/* Middle shelf */}
        <ZoneMesh zoneId="FridgeMiddleShelf" position={[0, size[1] / 2 - 1.8, 0]} size={[size[0] - 0.2, 1, size[2] - 0.3]} color="#e8e4e0" isSelected={selectedZoneId === "FridgeMiddleShelf"} onClick={onClick} label="Middle" />
        {/* Bottom shelf */}
        <ZoneMesh zoneId="FridgeBottomShelf" position={[0, size[1] / 2 - 3, 0]} size={[size[0] - 0.2, 1, size[2] - 0.3]} color="#e8e4e0" isSelected={selectedZoneId === "FridgeBottomShelf"} onClick={onClick} label="Bottom" />
        {/* Crispers */}
        <ZoneMesh zoneId="CrisperLeft" position={[-size[0] / 4, size[1] / 2 - 4.2, 0]} size={[size[0] / 2 - 0.1, 0.9, size[2] - 0.3]} color="#d4e8d4" isSelected={selectedZoneId === "CrisperLeft"} onClick={onClick} label="Crisper L" />
        <ZoneMesh zoneId="CrisperRight" position={[size[0] / 4, size[1] / 2 - 4.2, 0]} size={[size[0] / 2 - 0.1, 0.9, size[2] - 0.3]} color="#d4e8d4" isSelected={selectedZoneId === "CrisperRight"} onClick={onClick} label="Crisper R" />
        
        {/* Left door bins */}
        <ZoneMesh zoneId="FridgeLeftDoorTop" position={[-size[0] / 2 + 0.2, size[1] / 2 - 0.8, size[2] / 2 + 0.1]} size={[0.3, 0.8, 0.2]} color="#f0f0f0" isSelected={selectedZoneId === "FridgeLeftDoorTop"} onClick={onClick} label="L Door Top" />
        <ZoneMesh zoneId="FridgeLeftDoorMiddle" position={[-size[0] / 2 + 0.2, size[1] / 2 - 2, size[2] / 2 + 0.1]} size={[0.3, 0.8, 0.2]} color="#f0f0f0" isSelected={selectedZoneId === "FridgeLeftDoorMiddle"} onClick={onClick} label="L Door Mid" />
        <ZoneMesh zoneId="FridgeLeftDoorBottom" position={[-size[0] / 2 + 0.2, size[1] / 2 - 3.2, size[2] / 2 + 0.1]} size={[0.3, 0.8, 0.2]} color="#f0f0f0" isSelected={selectedZoneId === "FridgeLeftDoorBottom"} onClick={onClick} label="L Door Bot" />
        
        {/* Right door bins */}
        <ZoneMesh zoneId="FridgeRightDoorTop" position={[size[0] / 2 - 0.2, size[1] / 2 - 0.8, size[2] / 2 + 0.1]} size={[0.3, 0.8, 0.2]} color="#f0f0f0" isSelected={selectedZoneId === "FridgeRightDoorTop"} onClick={onClick} label="R Door Top" />
        <ZoneMesh zoneId="FridgeRightDoorMiddle" position={[size[0] / 2 - 0.2, size[1] / 2 - 2, size[2] / 2 + 0.1]} size={[0.3, 0.8, 0.2]} color="#f0f0f0" isSelected={selectedZoneId === "FridgeRightDoorMiddle"} onClick={onClick} label="R Door Mid" />
        <ZoneMesh zoneId="FridgeRightDoorBottom" position={[size[0] / 2 - 0.2, size[1] / 2 - 3.2, size[2] / 2 + 0.1]} size={[0.3, 0.8, 0.2]} color="#f0f0f0" isSelected={selectedZoneId === "FridgeRightDoorBottom"} onClick={onClick} label="R Door Bot" />
  ...
  if (type === 'base-drawer') {
    ...
    return (
      <group position={pos}>
        ...
        {/* Handle on drawer */}
        <mesh position={[...getHandleOffset().map((v, i) => i === 1 ? size[1] / 2 - drawerHeight / 2 : v) as [number, number, number]]} rotation={handleRotation}>
          <boxGeometry args={[size[0] * 0.3, 0.04, 0.02]} />
          <meshStandardMaterial color="#78716c" metalness={0.8} roughness={0.2} />
        </mesh>
        {/* Handle on door */}
        <mesh position={[...getHandleOffset().map((v, i) => i === 1 ? 0 : v) as [number, number, number]]} rotation={handleRotation}>
  ...
  if (unitId === 'STOVE') {
    return (
      <group position={pos}>
        <mesh onClick={(e) => { e.stopPropagation(); }} onPointerOver={(e) => { e.stopPropagation(); setHovered(true); }} onPointerOut={() => setHovered(false)}>
  ...
  return (
    <group position={pos}>
      <RoundedBox
        ...
        radius={0.03}
        smoothness={4}
        onClick={(e) => { e.stopPropagation(); if (zones[0]) onClick(zones[0]); }}
        onPointerOver={(e) => { e.stopPropagation(); setHovered(true); }}
        onPointerOut={() => setHovered(false)}
      >
        <meshStandardMaterial
